...
import static graphics.shape.shape2D.type.VfQuadCurve.*;
 ...
 *
 */
public class VfQuadCurveFXNodeFactoryImpl extends VfShapeTypeFXNodeFactoryBase<QuadCurve, VfQuadCurve> {
	static VfQuadCurveFXNodeFactoryImpl SINGLETON;
	
	/**
	 * 
	 * @return
	 */
	public static VfQuadCurveFXNodeFactoryImpl getSingleton() {
		if(SINGLETON==null) {
			SINGLETON=new VfQuadCurveFXNodeFactoryImpl();
		}
		return SINGLETON;
	}
    ...
    private final SimpleName controlXName;
    private final SimpleName controlYName;
	 ...
	 * private constructor
	 */
	private VfQuadCurveFXNodeFactoryImpl() {
		//initialize
        ...
        controlXName = PropertyTreeNodePathNameBuilder.start(CONTROL_PROPERTY_TREE.getName()).addNext(LAYOUT_CART_2D).addNext(LAYOUT_X).addNext(LAYOUT_COORD).build();
        controlYName = PropertyTreeNodePathNameBuilder.start(CONTROL_PROPERTY_TREE.getName()).addNext(LAYOUT_CART_2D).addNext(LAYOUT_Y).addNext(LAYOUT_COORD).build();
	...
	@Override
	public VfQuadCurve getVfShapeType() {
		return VfQuadCurve.SINGLETON;
	}
	 ...
	 */
	@Override
	public QuadCurve makeFXNode(Map<SimpleName, String> leafGraphicsPropertyFullPathNameOnTreeStringValueMap) {
		this.initialize();
		//
		...
		//
		boolean typeSpecificPropertySuccessfullySet = this.setTypeSpecificTreeProperty(quadCurve);
		
		if(typeSpecificPropertySuccessfullySet) {
			boolean transformSuccessfullySet = this.setTransform(quadCurve);
	...
	public void initialize() {
	    ...
	    controlX=LAYOUT_COORD.getDefaultValue();
	    controlY=LAYOUT_COORD.getDefaultValue();
	    //////////////////
	 ...
	 */
	@Override
	public boolean setTypeSpecificTreeProperty(QuadCurve quadCurve) {
		//START coordinate
		if(!this.setStartCoordinate(quadCurve))
			return false;
			
        //END coordinate
		if(!this.setEndCoordinate(quadCurve))
			return false;
        
		if(!this.setControl(quadCurve))
			return false;
		
		//color
		if(!this.setBorderColor(quadCurve))
			return false;
		
		//stroke
		if(!this.setStrokeWidth(quadCurve))
			return false;
		
		if(!this.setStrokeLineCap(quadCurve))
			return false;
		
		if(!this.setStrokeDashType(quadCurve))
			return false;
	 ...
	 * @return
	 */
	private boolean setStartCoordinate(QuadCurve quadCurve) {
		///////////////////startX
	 ...
	 * @return
	 */
	private boolean setEndCoordinate(QuadCurve quadCurve) {
		///////////////////endX
	...
	private boolean setControl(QuadCurve quadCurve) {
		///////////////////controlX1
		//extract non-null value for controlX1 if in the leafGraphicsPropertyFullPathNameOnTreeStringValueMap
		if(this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.containsKey(this.controlXName)
				&&this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.get(controlXName)!=null) {
			try {
				this.controlX = Double.parseDouble(this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.get(controlXName));
			}catch(NumberFormatException e) {
				return false;//the string value is invalid
			}
		}
		//if the property value is invalid, the shape instance cannot be created
		if(!LAYOUT_COORD.isValidValue(this.controlX))
			return false;
		...
		if(this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.containsKey(this.controlYName)
				&&this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.get(controlYName)!=null) {
			try{
				this.controlY = Double.parseDouble(this.leafGraphicsPropertyFullPathNameOnTreeStringValueMap.get(controlYName));
			}catch(NumberFormatException e) {
				return false;//the string value is invalid
			}
		}
		//if the property value is invalid, the shape instance cannot be created
		if(!LAYOUT_COORD.isValidValue(this.controlY))
			return false;
		
		//only set the property value if not null; otherwise, do nothing (probably a default value will be set in JAVAFX);
		if(this.controlY!=null)
	 ...
	 * @return
	 */
	private boolean setBorderColor(QuadCurve quadCurve) {
		//red
	 ...
	 * @return
	 */
	private boolean setStrokeWidth(QuadCurve quadCurve) {
		...
		if(this.strokeWidth!=null)
			quadCurve.setStrokeWidth(this.strokeWidth);
	 ...
	 * @return
	 */
	private boolean setStrokeLineCap(QuadCurve quadCurve) {
		...
		if(this.strokeLineCap!=null)
			quadCurve.setStrokeLineCap(this.strokeLineCap);
	 ...
	 * @return
	 */
	private boolean setStrokeDashType(QuadCurve quadCurve) {
		...
		if(this.dashType!=null)
			quadCurve.getStrokeDashArray().addAll(this.dashType.getDashArrayList());
